...
export function JoinRequestQueue() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("inbox.title"), href: "/inbox" },
      { label: t("joinRequestQueue.title") },
    ]);
  ...
  if (requestsQuery.error) {
    const message =
      requestsQuery.error instanceof ApiError && requestsQuery.error.status === 403
        ? t("joinRequestQueue.permissionDenied")
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex flex-wrap gap-3 rounded-xl border border-border bg-card p-4">
        <label className="space-y-2 text-sm">
          ...
          >
            <option value="pending_approval">{t("joinRequestQueue.filters.pendingApproval")}</option>
            <option value="approved">{t("joinRequestQueue.filters.approved")}</option>
            <option value="rejected">{t("joinRequestQueue.filters.rejected")}</option>
          </select>
        </label>
        <label className="space-y-2 text-sm">
          <span className="font-medium">{t("joinRequestQueue.filters.requestType")}</span>
          <select
            className="rounded-md border border-border bg-background px-3 py-2"
            value={requestType}
            onChange={(event) =>
          ...
          >
            <option value="all">{t("joinRequestQueue.filters.all")}</option>
            <option value="human">{t("joinRequestQueue.filters.human")}</option>
            <option value="agent">{t("joinRequestQueue.filters.agent")}</option>
          </select>
        </label>
      </div>
            ...
            <div key={request.id} className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  ...
                  <div className="mt-2">
                    {request.invite
                      ? `${request.invite.allowedJoinTypes} ${t("joinRequestQueue.joinInvite")}${request.invite.humanRole ? ` • ${t("joinRequestQueue.defaultRole")} ${request.invite.humanRole}` : ""}`
