...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  const title = scope === "invalid_company_prefix" ? t("notFound.companyNotFound.title") : t("notFound.pageNotFound.title");
  const description =
    scope === "invalid_company_prefix"
      ? `${t("notFound.companyNotFound.description")} "${normalizedPrefix ?? t("common.labels.unknown")}".`
      : t("notFound.pageNotFound.description");
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Button asChild>
            <Link to={dashboardHref}>
              ...
              {t("notFound.actions.openDashboard")}
            </Link>
          </Button>
