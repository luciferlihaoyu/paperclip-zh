...
export function NewAgent() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("newAgent.breadcrumb.agents"), href: "/agents" },
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : "{t("newAgent.errors.createFailed")}");
    },
  });
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      if (!isValidOpenCodeModelId(configValues.model)) {
        setFormError("{t("newAgent.errors.opencodeModelRequired")}");
        return;
      }
    }
    createAgent.mutate(
      ...
      <div className="border border-border">
        ...
        <div className="px-4 pt-4 pb-2">
          <input
            ...
            placeholder={t("newAgent.fields.namePlaceholder")}
            value={name}
        ...
        <div className="px-4 pb-2">
          <input
            ...
            placeholder={t("newAgent.fields.titlePlaceholder")}
            value={title}
        ...
        <div className="border-t border-border px-4 py-4">
          <div className="space-y-3">
            <div>
              <h2 className="text-sm font-medium">{t("newAgent.sections.companySkills")}</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("newAgent.sections.companySkillsHint")}
              </p>
            </div>
            {availableSkills.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                {t("newAgent.empty.noCompanySkills")}
              </p>
            ) : (
              <div className="space-y-3">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("newAgent.alert.firstAgentCeo")}</p>
          )}
          {formError && (
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {testAgentState.pending ? {t("newAgent.actions.testing")} : {t("newAgent.actions.testAgent")}}
                </Button>
                <Button
                  size="sm"
                ...
                >
                  {createAgent.isPending ? {t("newAgent.actions.creating")} : {t("newAgent.actions.createAgent")}}
                </Button>
              </div>
            </div>
          </div>
