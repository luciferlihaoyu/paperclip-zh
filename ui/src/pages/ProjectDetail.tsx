  ...
  return (
    <div className="space-y-6">
      <InlineEditor
        ...
        placeholder=t("projectDetail.overview.addDescription")
        multiline
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-label={t("projectDetail.overview.changeColor")}
      />
      {open && (
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {PROJECT_COLORS.map((color) => (
              <button
                ...
                aria-label={`${t("projectDetail.overview.selectColor")} ${color}`}
              />
            ))}
          </div>
        </div>
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      ...
      if (archived) {
        pushToast({ title: `"${name}" ${t("projectDetail.status.archived")}`, tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: `"${name}" ${t("projectDetail.status.unarchived")}`, tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
        title: archived ? t("projectDetail.errors.archiveFailed") : t("projectDetail.errors.unarchiveFailed"),
        tone: "error",
      });
    },
  });
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("projectDetail.breadcrumb.projects"), href: "/projects" },
      { label: project?.name ?? routeProjectRef ?? t("projectDetail.breadcrumb.defaultProject") },
    ]);
    ...
    <div className="space-y-6">
      {showLeftProjectNotice ? (
        <div className="flex items-center gap-3 border border-yellow-300/35 bg-yellow-300/10 px-3 py-2 text-sm text-yellow-100">
          <p className="min-w-0 flex-1">
            {t("projectDetail.alert.leftProject")}
          </p>
          <MembershipAction
            compact
            state="left"
          ...
          <button
            ...
            aria-label=t("projectDetail.alert.dismissNotice")
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.pauseReason === "budget" ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-red-200">
              ...
              {t("projectDetail.alert.pausedByBudget")}
            </div>
          ) : null}
          {project.managedByPlugin ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-[11px] font-medium text-muted-foreground">
              ...
              {t("projectDetail.alert.managedBy")} {project.managedByPlugin.pluginDisplayName}
            </div>
          ) : null}
        </div>
      </div>
      ...
      {activeTab === "workspaces" ? (
        workspaceTabDecisionLoaded ? (
          ...
          <p className="text-sm text-muted-foreground">{t("projectDetail.loading.workspaces")}</p>
        )
      ) : null}
