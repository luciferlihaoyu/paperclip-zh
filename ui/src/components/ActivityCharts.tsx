...
function useActivityChartsTranslation() {
  const { t } = useTranslation();
  return t;
}

export function RunActivityChart(props: RunChartProps) {
  const t = useActivityChartsTranslation();
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("components.activityCharts.noRuns")}</p>;
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("components.activityCharts.noIssues")}</p>;
...
const STATUS_LABEL_KEYS: Record<string, string> = {
  todo: "components.activityCharts.todo",
  in_progress: "components.activityCharts.inProgress",
  in_review: "components.activityCharts.inReview",
  done: "components.activityCharts.done",
  blocked: "components.activityCharts.blocked",
  cancelled: "components.activityCharts.cancelled",
  backlog: "components.activityCharts.backlog",
};
